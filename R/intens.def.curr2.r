...
intens.def.curr = function(curr.outbreak, params, tbls, preoutbreak, outbreak, collapse, calm){
  ...
  if(collapse>=4){ #Quim: ho faig perquè el collapse sigui més suau al principi
    to.collapse = rdunif(1, 0.05*nrow(active.defol), 0.15*nrow(active.defol)) #Valors calibrats amb les dades històriques
  ...
  if(collapse>1 & collapse<4){
    ...
    active.defol$new.intens[active.defol$new.intens==1 & active.defol$ny.def>1] = 
      sample(1:3, size=length(active.defol$new.intens[active.defol$new.intens==1 & active.defol$ny.def>1]), replace=T, prob=c(0.7,0.2,0.1)) #*size=length(active.defol$new.intens==1 & active.defol$ny.def>1)
  }
  else if(calm>1){
    to.collapse = rdunif(1, 0.2*nrow(active.defol), 0.6*nrow(active.defol))
