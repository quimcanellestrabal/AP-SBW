...
#' @param suitab A data frame with climatic suitability and soil suitability for potential colonizing species for each forest stand. It is the result of the function \code{suitability()}
...
#' land=landscape; target.cells=landscape$cell.id[runif(10,1,nrow(landscape))]; tbls=default.tables; type.trans = "S" 
#' forest.transition(landscape, landscape$cell.id[runif(10,1,nrow(landscape))], suitab, params, tbls=default.tables, type.trans = "S")
#' 
...
forest.transition = function(land, target.cells, suitab, params, tbls, type.trans = "S"){   
  ...
  post.harvest.reg = tbls$post.harvest.reg
  ...
  ## Eufeuillement volontaire suite aux coupes, that is, after clear-cut for a % of EPN and SAB to transform to PET ##*** ho mantinc??
  if(type.trans=="C" & params$enfeuil>0){  
    vec.enfeuil = filter(subland, spp %in% c("EPN", "SAB") & potential.spp=="PET") %>% dplyr::select(ptrans)
